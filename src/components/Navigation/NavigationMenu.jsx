...
import ProfileIcon from "../../assets/icons/core/account_icon.svg";
...
const NavigationMenu = () => {
	...
	const links = [
		...
		{ icon: DiscoverIcon, label: "Explorar", link: "/" },
		...(isAuthenticated
			? [
				{
					icon: user?.photoURL || ProfileIcon,
					label: "Perfil",
					link: "/",
		...
		<div className="flex relative lg:flex-col w-full justify-between items-center pb-[2vh] lg:pb-[0]  lg:w-[15vw] lg:max-w-64 ">
			...
			<nav className="flex flex-row lg:flex-col flex-grow  items-center justify-center space-x-3 lg:space-x-0 lg:space-y-3 w-full lg:mb-4">
				{links.map((item, index) => (
					...
					<NavLink key={index} className="w-[70px] lg:w-full h-[50px] flex items-center justify-center rounded-full cursor-default ">
						<div className="navbarLink absolute z-10 lg:w-[104px] text-negro-carbon flex items-center  cursor-pointer">

							{item.icon === user?.photoURL ? (
